...
export async function handleGroups(req, path) {
    ...
    `).get(body.name.trim(), body.avatarUrl || null, user.sub);
        // Add creator as owner
        const insertMember = db.prepare("INSERT INTO group_members (group_id, member_id, member_type, role) VALUES (?, ?, ?, ?)");
        insertMember.run(group.id, user.sub, "user", "owner");
        // Add initial members
        if (body.memberIds?.length) {
            const tx = db.transaction(() => {
                for (const m of body.memberIds) {
                    if (m.id === user.sub && m.type === "user")
                        continue; // skip creator duplicate
    ...
    if (detailMatch && req.method === "GET") {
        ...
        // Verify membership
        const isMember = db.prepare("SELECT 1 FROM group_members WHERE group_id = ? AND member_id = ? AND member_type = 'user'").get(groupId, user.sub);
        if (!isMember)
    ...
    if (patchMatch && req.method === "PATCH") {
        ...
        const membership = db.prepare("SELECT role FROM group_members WHERE group_id = ? AND member_id = ? AND member_type = 'user'").get(groupId, user.sub);
    ...
    if (addMemberMatch && req.method === "POST") {
        ...
        try {
            db.prepare("INSERT INTO group_members (group_id, member_id, member_type) VALUES (?, ?, ?)").run(groupId, body.memberId, body.memberType);
        }
        catch {
    ...
    if (removeMemberMatch && req.method === "DELETE") {
        ...
        db.prepare("DELETE FROM group_members WHERE group_id = ? AND member_id = ? AND member_type = ?").run(groupId, memberId, memberType);
        return Response.json({ ok: true });
    }
    ...
    if (leaveMatch && req.method === "POST") {
        ...
        db.prepare("DELETE FROM group_members WHERE group_id = ? AND member_id = ? AND member_type = 'user'").run(groupId, user.sub);
        return Response.json({ ok: true });
    }
    ...
    if (deleteMatch && req.method === "DELETE") {
        ...
        if (!group)
            return Response.json({ error: "group not found" }, { status: 404 });
        if (group.created_by !== user.sub)
            return Response.json({ error: "only creator can delete" }, { status: 403 });
        const tx = db.transaction(() => {
