...
export async function handleBots(req, path) {
    ...
    if (path === "/api/bots" && req.method === "POST") {
      ...
      RETURNING id, name, avatar_url as "avatarUrl",
                is_public as "isPublic", is_online as "isOnline"
    `).get(user.sub, body.name.trim(), body.avatarUrl || null, body.gatewayUrl.trim(), body.gatewayToken || null, body.isPublic !== false ? 1 : 0);
    ...
    if (patchMatch && req.method === "PATCH") {
        ...
        // Verify ownership
        const bot = db.prepare("SELECT owner_id FROM bots WHERE id = ?").get(botId);
        if (!bot)
            return Response.json({ error: "bot not found" }, { status: 404 });
        if (bot.owner_id !== user.sub)
        ...
        if (body.gatewayUrl !== undefined) {
            fields.push("gateway_url = ?");
            values.push(body.gatewayUrl);
        }
        if (body.gatewayToken !== undefined) {
            fields.push("gateway_token = ?");
            values.push(body.gatewayToken);
        }
    ...
    if (testMatch && req.method === "POST") {
        ...
        const bot = db.prepare("SELECT gateway_url, gateway_token FROM bots WHERE id = ? AND (owner_id = ? OR is_public = 1)").get(botId, user.sub);
        if (!bot)
            return Response.json({ error: "bot not found" }, { status: 404 });
        try {
