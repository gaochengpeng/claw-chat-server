 ...
 */
export async function getGatewayConnection(botId) {
    ...
    // 从数据库获取 Bot 信息
    const db = getDB();
    const bot = db.prepare("SELECT gateway_url, gateway_token FROM bots WHERE id = ?").get(botId);
    if (!bot)
        throw new Error(`Bot ${botId} not found`);
    return createConnection(botId, bot.gateway_url, bot.gateway_token);
}
async function createConnection(botId, gatewayUrl, gatewayToken) {
    // 清理旧连接
...
function scheduleReconnect(conn, gatewayToken) {
    if (conn.reconnectTimer)
        return;
    const delay = Math.min(BASE_RECONNECT_DELAY * Math.pow(2, conn.reconnectAttempts), MAX_RECONNECT_DELAY);
 ...
 */
export function sendToGateway(conn, message, onChunk) {
    return new Promise((resolve, reject) => {
 ...
 * 断开 Bot 连接
 */
export function disconnectBot(botId) {
...
function updateBotOnlineStatus(botId, online) {
    try {
        const db = getDB();
